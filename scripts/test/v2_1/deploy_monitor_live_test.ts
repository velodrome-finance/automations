...
async function main() {
  ...
  const underfundedUpkeepAmount = ethers.utils.parseEther('1')
  ...
  const balanceMonitorFundAmount = ethers.utils.parseEther('10')
  ...
  const balanceMonitorUpkeepId = await registerCustomLogicUpkeep(
    automationRegistrar,
